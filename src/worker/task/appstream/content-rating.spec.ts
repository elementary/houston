...
test('missing content_rating causes an error', async () => {
  ...
  await worker.setup()
  await worker.run(AppstreamContentRating)
  await worker.teardown()

  expect(worker.fails()).toBeTruthy()
  expect(worker.storage.logs[0].title.toLowerCase()).toContain('missing')
...
test('missing a content_rating attribute causes an error', async () => {
  ...
  expect(worker.fails()).toBeTruthy()
  expect(worker.storage.logs[0].body.toLowerCase()).toContain('sex-adultery')
  expect(worker.storage.logs[0].body.toLowerCase()).toContain('sex-appearance')
  expect(worker.storage.logs[0].body.toLowerCase()).toContain('social-chat')
  expect(worker.storage.logs[0].body.toLowerCase()).toContain('social-audio')
}, 300000) // A 5 minute timeout
