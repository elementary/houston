...
describe('lang', () => {
  it('can convert based on array length', (done) => {
    ...
    assert.equal(test1, 'all thing to singular')
    assert.equal(test2, 'too many person')
    assert.equal(test3, 'people things are cats dogs')
    assert.equal(test4, 'the numbers in arrays')
    done()
  })
})
