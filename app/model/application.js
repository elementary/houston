...
ApplicationSchema.methods.toClean = function() {
  ...
  if (typeof application.icon === 'string') { // Trip super long encoded data
...
ApplicationSchema.methods.upsertRelease = function(query, object) {
  ...
  const notDotQuery = _.mapValues(dotQuery, value => {
    return { $ne: value };
  });
  ...
  return Promise.all([ // Return two promises (need mongodb $setOnUpdate for single function)
    ...
    Application.update(_.extend({ // Find an application that has _id but not release
      _id: application._id,
    }, notDotQuery), {
      $addToSet: {
...
ApplicationSchema.methods.testUpdate = function() {
  const application = this;
